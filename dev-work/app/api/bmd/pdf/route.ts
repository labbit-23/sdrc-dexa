...
function buildFilename(patientId: string, suffix?: string): string {
  try {
    ...
    return `${name}_Body_Composition_${date}${suffix ? '_' + suffix : ''}.pdf`
  } catch {
    return `${safe(patientId)}_Body_Composition${suffix ? '_' + suffix : ''}.pdf`
  }
}
...
export async function GET(req: NextRequest) {
  ...
  const reportUrl = `${proto}://${host}/bmd/render/${patientId}?dark=0${mode}`
